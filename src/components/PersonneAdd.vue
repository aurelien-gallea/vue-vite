<template>
	...
	<Form @submit="ajouterPersonne" :validation-schema="schema" v-slot="{ meta, errors }">
		<ul>
			<li v-for="(v, k) in errors">{{ k }} : {{ v }}</li>
		</ul>
		<div>
...
export default {
	...
	data() {
		const validateString = () =>
			yup
				...
				.matches(
					/^[A-Z]{1}.*/,
					(params) => `'${params.value}' ne commence pas par une majuscule`
				);
		return {
			schema: yup.object({
				nom: validateString(),
	...
	methods: {
		ajouterPersonne(values, actions) {
            ...
            .then(res => this.$emit('addPersonne', res.data))
			actions.resetForm();
		},
